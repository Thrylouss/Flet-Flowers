...
def update_profile(user, token, page):
    ...
    response = requests.put(
        f"{API_BASE_URL}/user/update_user",
        headers={
...
def show_profile_page(page, user):
    ...
    def save_changes(e):
        ...
        if new_username != user['name'] or new_email != user['email'] or new_password != user['password']:
            # Если есть изменения, то обновляем данные профиля
